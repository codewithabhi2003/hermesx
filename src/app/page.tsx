...
import { Zap, Sparkles, LayoutGrid, MessageSquareText, ArrowRight, MessageSquare, ThumbsUp, Minus, ThumbsDown } from 'lucide-react';
...
/** Slow-drifting blurred gradient orbs behind the hero — purely decorative, respects reduced-motion via the global CSS rule in globals.css. */
...
const HERO_THEMES = [
  { label: 'Feature Requests', pct: 92, color: '#3B82F6' },
  { label: 'Customer Support', pct: 78, color: '#10B981' },
  { label: 'Performance', pct: 58, color: '#EF4444' },
  { label: 'Onboarding', pct: 44, color: '#6366F1' },
  { label: 'Billing', pct: 32, color: '#F59E0B' },
] as const;

const TREND_POINTS = '0,50 20,35 40,48 60,20 80,40 100,12 120,45 140,25 160,52 180,18 200,42 220,8 240,38 260,48 280,22 300,35';

/**
 * A scaled-down mockup of the real dashboard's stats/chart/themes — kept
 * in sync with the SITE'S theme (via useTheme), not hard-coded to light.
 * Deliberately has no top bar section — just the data visualization part.
 */
function HeroPreview() {
  ...
  const cardClass = isNight ? 'border-white/10 bg-[#0A0A0A]' : 'border-gray-200 bg-white';
  const innerClass = isNight ? 'border-white/10 bg-white/5' : 'border-gray-100 bg-gray-50';
  const labelClass = isNight ? 'text-slate-400' : 'text-gray-500';
  const headingClass = isNight ? 'text-slate-200' : 'text-gray-700';
  const trackClass = isNight ? 'bg-white/10' : 'bg-gray-100';

  const stats = [
    { label: 'Total', value: '1,248', icon: MessageSquare, color: isNight ? '#E2E8F0' : '#111827' },
    { label: 'Positive', value: '848', icon: ThumbsUp, color: colors.positive },
    { label: 'Neutral', value: '275', icon: Minus, color: colors.neutral },
    { label: 'Negative', value: '125', icon: ThumbsDown, color: colors.negative },
  ];

  return (
    <div className="relative animate-slide-up">
      <div className={`overflow-hidden rounded-2xl border p-5 shadow-2xl ${cardClass}`}>
        {/* Stat cards */}
        <motion.div
        ...
        >
          {stats.map((stat) => (
            ...
            >
              <stat.icon className={`mb-1 h-3 w-3 ${labelClass}`} />
              <p className="text-base font-bold leading-tight" style={{ color: stat.color }}>
                {stat.value}
              </p>
              <p className={`text-[9px] ${labelClass}`}>{stat.label}</p>
            </motion.div>
          ))}
        </motion.div>

        {/* Trend chart — draws itself in on mount */}
        <div className="mb-5">
          <p className={`mb-2 text-[11px] font-semibold ${headingClass}`}>Feedback Trend</p>
        ...
        {/* Top themes — bars grow in with a stagger */}
        <div>
          <p className={`mb-2 text-[11px] font-semibold ${headingClass}`}>Top Themes</p>
          <div className="space-y-1.5">
            {HERO_THEMES.map((themeItem, i) => (
              <div key={themeItem.label} className="flex items-center gap-2">
                <span className={`w-[74px] flex-shrink-0 truncate text-[9px] ${labelClass}`}>{themeItem.label}</span>
                <div className={`h-2 flex-1 rounded-full ${trackClass}`}>
                  <motion.div
                    className="h-2 rounded-full"
                    style={{ backgroundColor: themeItem.color }}
                    initial={{ width: 0 }}
                    animate={{ width: `${themeItem.pct}%` }}
      ...
      >
        <p className={`text-xs ${labelClass}`}>Top theme this week</p>
        <p className={`text-sm font-semibold ${isNight ? 'text-slate-50' : 'text-gray-900'}`}>Feature Requests</p>
      </motion.div>
    </div>
  );
}
        ...
        <div className="grid items-center gap-16 lg:grid-cols-2">
          <div className="animate-fade-in">
            ...
            <p className="mt-6 max-w-xl text-lg text-text-secondary">
              Understand what customers love, what frustrates them, and what they want next — with insights
              grounded in the feedback they actually shared.
            </p>
      ...
      <footer className="border-t border-border bg-surface">
        <div className="mx-auto max-w-7xl px-6 py-16">
          <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
            <div className="lg:col-span-2">
              ...
              <p className="mt-4 max-w-xs text-sm text-text-secondary">
                AI-powered customer feedback intelligence — understand what your customers are saying and turn it
                into action.
              </p>
            </div>

            <div>
              <h3 className="text-xs font-semibold uppercase tracking-wider text-text-muted">Product</h3>
            ...
            <div>
              <h3 className="text-xs font-semibold uppercase tracking-wider text-text-muted">Account</h3>
              <ul className="mt-4 space-y-3">
                <li>
                ...
                <li>
                  <Link href="/signup" className="text-sm text-text-secondary hover:text-text-primary">
                    Get started
                  </Link>
                </li>
              </ul>
          ...
          <div className="mt-14 flex flex-col items-center justify-between gap-4 border-t border-border pt-8 sm:flex-row">
            <p className="text-xs text-text-muted">© {new Date().getFullYear()} HermesX. All rights reserved.</p>
